...
NrSlUeMacSchedulerSimple::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrSlUeMacSchedulerSimple")
    ...
    .AddAttribute ("OReExp",
                   ...
                   BooleanValue (false), 
                   MakeBooleanAccessor (&NrSlUeMacSchedulerSimple::m_OReExp),
                   MakeBooleanChecker ())
  ;
  return tid;
}
                                            ...
                                            std::set<NrSlSlotAlloc> &slotAllocList)
{
  ...
  if (m_OReExp)
  {
    ...
    std::ifstream f("../../../encodedSelectionInstructions.txt");
    ...
    std::cout<<encodedSelectionInstructionsString<<std::endl;
...
std::list <NrSlUeMacSchedSapProvider::NrSlSlotInfo>
NrSlUeMacSchedulerSimple::RandomlySelectSlots (std::list <NrSlUeMacSchedSapProvider::NrSlSlotInfo> txOpps, uint8_t totalTx)
{
  NS_LOG_FUNCTION (this);
