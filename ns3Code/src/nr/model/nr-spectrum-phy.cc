...
NrSpectrumPhy::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("SensingThreshold",
                   "The energy of a received signal should be higher than "
                   "this threshold (dbm) to allow the PHY layer to declare CCA BUSY state.",
                    DoubleValue (4.5*pow(10,-17)),
                    MakeDoubleAccessor (&NrSpectrumPhy::SetSensingThreshold,
                                       &NrSpectrumPhy::GetSensingThreshold),
                    MakeDoubleChecker<double> ())
...
void
NrSpectrumPhy::SetSensingThreshold (double threshold)
{
  NS_LOG_FUNCTION (this << threshold);
  m_sensingThreshold = threshold;
}

double
NrSpectrumPhy::GetSensingThreshold (void) const
{
  return m_sensingThreshold;
}
...
NrSpectrumPhy::RxSlPscch (std::vector<uint32_t> paramIndexes)
{
  ...
  std::list<PscchPduInfo> rxControlMessageFailList;
  bool error = true;
  ...
  if (m_sensingEnabled)
    {
      SpectrumValue slSigPerceivedTemp;
      for (std::multiset<SlCtrlSigParamInfo>::iterator it = sortedControlMessages.begin (); it != sortedControlMessages.end (); it++ )//collect the total power in each RB
        {
          uint32_t pktIndex = (*it).index;
          
          if (it == sortedControlMessages.begin ())
          {
            slSigPerceivedTemp = m_slSigPerceived[pktIndex];
          }
          else
          {
            slSigPerceivedTemp += m_slSigPerceived[pktIndex];
      ...
      for (Values::const_iterator rbIt =  slSigPerceivedTemp.ValuesBegin(); rbIt != slSigPerceivedTemp.ValuesEnd(); rbIt++)
        {
          if (*rbIt < m_sensingThreshold)
            {
              collidedRbBitmap.insert (rbIt - slSigPerceivedTemp.ValuesBegin());
            }
        }
    }
  ...
  for (auto &ctrlMsgIt : sortedControlMessages)
    {
      ...
      if (!corrupt)
      ...
      else if (corrupt)
        {
          ...
          pduInfo.packet = packet;
          pduInfo.psd = psd;
          rxControlMessageFailList.push_back (pduInfo);
          //rbDecodedBitmap.insert ( m_slRxSigParamInfo.at (paramIndex).rbBitmap.begin (), m_slRxSigParamInfo.at (paramIndex).rbBitmap.end ()); 
        }
      ...
      if (!error)
        {
          ...
          for (it = rxControlMessageOkList.begin (); it != rxControlMessageOkList.end (); it++)
            {
              m_nrPhyRxPscchEndOkCallback (it->packet, it->psd,false);
            }

          std::list<PscchPduInfo>::iterator it2;
          for (it2 = rxControlMessageFailList.begin (); it2 != rxControlMessageFailList.end (); it2++)
            {
              NrSlSciF1aHeader sciF1a;
              m_nrPhyRxPscchEndOkCallback (it2->packet, it2->psd,true);
            }
        }
        else if (error)
          {
            NS_LOG_DEBUG (this << " PSCCH not OK");
            std::list<PscchPduInfo>::iterator it;
            for (it = rxControlMessageFailList.begin (); it != rxControlMessageFailList.end (); it++)
            {
              m_nrPhyRxPscchEndOkCallback (it->packet, it->psd,true);
            }
          }
    }
}
  ...
  for (uint32_t i = 0; i < m_slRxSigParamInfo.size (); i++)
    {
      ...
      //bool foo = itTb == m_slTransportBlocks.end ();
