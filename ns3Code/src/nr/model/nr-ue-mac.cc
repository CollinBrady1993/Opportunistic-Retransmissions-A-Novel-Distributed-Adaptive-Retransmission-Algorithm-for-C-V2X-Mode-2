  ...
  static TypeId tid = TypeId ("ns3::NrUeMac")
    ...
    .AddAttribute ("ReservationPeriod",
     ...
     .AddAttribute ("OReExp",
                   ...
                   BooleanValue (false), 
                   MakeBooleanAccessor (&NrUeMac::m_OReExp),
                   MakeBooleanChecker ())
...
NrUeMac::NrUeMac (void) : Object ()
{
  ...
  m_ueSelectedExponentialVariable = CreateObject<ExponentialRandomVariable> ();
  m_nrSlHarq = CreateObject<NrSlUeMacHarq> ();
  m_OReEnv = Create<OREENV> (2333);
}
...
NrUeMac::DoDispose ()
{
  ...
  m_ueSelectedExponentialVariable = nullptr;
...
NrUeMac::AssignStreams (int64_t stream)
{
  ...
  m_ueSelectedExponentialVariable->SetStream (stream + 2);
  return 3;
}
//NR SL
...
NrUeMac::GetNrSlTxOpportunities (const SfnSf& sfn)
{
  ...
  uint32_t mTotal = GetTotalSubCh (m_poolId)*candSsResoA.size (); // total number of candidate single-slot resources
  ...
  if (m_enableSensing)
    {
      ...
      std::list<std::list<SlotSensingData>> allSensingData;
      for (const auto &itSensedSlot:sensedData)
        {
          ...
          allSensingData.emplace_back (listFutureSensTx);
        }
      ...
      do
        {
          ...
          while (itCandSsResoA != nrCandSsResoA.end ())//iterate over candidates
            {
              bool erased = false;
              ...
              for (const auto &itSensedSlot:allSensingData)
                {
                  std::list<SlotSensingData> listFutureSensTx = itSensedSlot;
      ...
      while (eliminatedResources > (1 - (GetResourcePercentage () / 100.0)) * mTotal);//(nrCandSsResoA.size () < (GetResourcePercentage () / 100.0) * mTotal);
      ...
      if (m_OReExp)
      {
        ...
        std::ofstream ofs1("../../../encodedSrcRnti.txt", std::ofstream::out);
        ofs1 << encodedSrcRnti <<std::endl;
        ofs1.close();

        std::ofstream ofs2("../../../encodedRc.txt", std::ofstream::out);
        ofs2 << encodedRc <<std::endl;
        ofs2.close();

        std::ofstream ofs3("../../../encodedSrcSlot.txt", std::ofstream::out);
        ofs3 << encodedSrcSlot <<std::endl;
        ofs3.close();

        std::ofstream ofs4("../../../encodedSrcSc.txt", std::ofstream::out);//std::ofstream::trunc);
  ...
  for (uint16_t i = 0; i <= q; i++)
    {
      SlotSensingData sensedSlotData (sensedData.sfn, sensedData.rsvp,
                                      ...
                                      sensedData.prio, sensedData.slRsrp,sensedData.corrupt,sensedData.Rc,sensedData.srcRnti);
...
void
NrUeMac::DoNrSlSlotIndication (const SfnSf& sfn) //touchPoint, this is the function where packets are transmitted and new grants are generated. It also has access to the sensing info which is all we need for D-Re.
{
      ...
      if (itGrantInfo.second.slResoReselCounter >= 0 && itGrantInfo.second.slotAllocations.begin ()->sfn == sfn)
        {
          ...
          //make DRE adjustements if needed
          ...
          if (currentGrant.txSci1A)
            {
              ...
              sciF1a.SetRc(itGrantInfo.second.slResoReselCounter);
...
NrUeMac::CreateGrantInfo (const std::set<NrSlSlotAlloc>& slotAllocList)
{
  ...
  NrSlGrantInfo grant;
  resPeriodSlots = m_slTxPool->GetResvPeriodInSlots (GetBwpId (), m_poolId, m_pRsvpTx, m_nrSlUePhySapProvider->GetSlotPeriod ());
  ...
  for (uint16_t i = 0; i < grant.cReselCounter; i++)
    {
...
NrUeMac::GetRndmReselectionCounter () const
{
  ...
  switch(periodInt)
  {
    ...
    case 950:
    case 1000:
      min = 5;//5;//this is a hacky solution to a bug in the code, rather than decrement the counter during the last packet (per set of HARQ packets) we instead do it at the first packet and increase the bounds of the counter by 1, this recreates the intended counter effects
      max = 15;//15;
      break;
    default:
      if (periodInt < 100)
