...
public:
  ...
  /**
   * \brief Set Sensing threshold
   * \param threshold - Sensing threshold
   */
  void SetSensingThreshold (double threshold);
  /**
   * Returns Sensing threshold
   * \return Sensing threshold
   */
  double GetSensingThreshold (void) const;
  /**
...
public:
 ...
 struct SlTransportBlockInfo
 {
   ...
   */
  typedef std::function<void (const Ptr<Packet>&, const SpectrumValue&, const bool&)> NrPhyRxPscchEndOkCallback;
  /**
...
private:
  ...
  bool m_sensingEnabled {true}; //!<whether sensing is enabled by UE in SL.
  double m_sensingThreshold; //!< threshold for sening if enabled. If the total power in an RB is below m_sensingThreshold then the RB is considered corrupted
