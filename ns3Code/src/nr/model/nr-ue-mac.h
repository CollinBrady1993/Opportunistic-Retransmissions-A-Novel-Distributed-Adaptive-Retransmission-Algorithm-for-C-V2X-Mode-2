...
private:
  ...
  struct NrSlGrantInfo
  {
    ...
    int slResoReselCounter {std::numeric_limits <uint8_t>::max ()}; //!< The Sidelink resource re-selection counter for the semi-persistently scheduled resources as per TS 38.214
  ...
  Ptr<ExponentialRandomVariable> m_ueSelectedExponentialVariable; //!< exponential random variable used for NR Sidelink
  ...
  uint8_t m_resPercentage {0}; /**< The percentage threshold to indicate the
                                    ...
                                    */
  int m_reselCounter {0}; //!< The resource selection counter
  uint16_t m_cResel {0}; //!< The C_resel counter
  /** 
  *   Flag for if the experiment uses the ORe experimental settings, cannot be on with m_DReExp
  */
  bool m_OReExp;
  /**
