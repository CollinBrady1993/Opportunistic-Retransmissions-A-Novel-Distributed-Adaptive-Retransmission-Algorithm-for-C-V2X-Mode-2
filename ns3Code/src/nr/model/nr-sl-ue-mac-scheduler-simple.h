...
private:
  ...
  std::list <NrSlUeMacSchedSapProvider::NrSlSlotInfo>
  /**
   * \brief Randomly select the number of slots from the slots given by UE MAC, used only in O-Re experiments
   *
   * If K denotes the total number of available slots, and N_PSSCH_maxTx is the
   * maximum number of PSSCH configured transmissions, then:
   *
   ...
   *
   * \param txOpps The list of the available slots
   * \param totalTx is a preset number used rather than drawing the defined NSe from elsewhere in the simulation
   * \return The list of randomly selected slots
   */
  RandomlySelectSlots (std::list <NrSlUeMacSchedSapProvider::NrSlSlotInfo> txOpps, uint8_t totalTx);
  /**
